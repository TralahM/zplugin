...
echo ">>> Downloading TralahM/zplugin module to $ZPLG_HOME/mod-bin"
if test -d "$ZPLG_HOME/mod-bin/.git"; then
    cd "$ZPLG_HOME/mod-bin"
    git pull origin master
else
    cd "$ZPLG_HOME"
    git clone --depth 10 https://github.com/TralahM/zplugin.git mod-bin
fi
echo ">>> Done"
...
echo "$col_pname== Building module TralahM/zplugin, running: a make clean, then ./configure and then make ==$col_rst"
...
CPPFLAGS=-I/usr/local/include CFLAGS="-g -Wall -O3" LDFLAGS=-L/usr/local/lib ./configure --disable-gdbm && {
  ...
  make && {
    ...
    echo "while the module is loaded (check with Zsh command \`zmodload')."
      ...
      echo "error-report at: https://github.com/TralahM/zplugin/issues"
  }
}
